...
contract TransferFrom is Helper {
    address sender = makeAddr("TransferFrom: transfer sender");
    ...
    function test_succeeds_when_internal_call_succeeds() public {
        ...
        vm.mockCall(
            address(standardToken), abi.encodeCall(IERC20.transferFrom, (sender, recipient, amount)), abi.encode(true)
        );
    ...
    function test_does_not_revert_when_the_internal_call_has_no_return_data() public {
        ...
        vm.mockCall(
            address(tokenNoReturnValue), abi.encodeCall(IERC20.transferFrom, (sender, recipient, amount)), hex""
        );
    ...
    function test_reverts_when_too_much_data_is_returned() public {
        ...
        executor.transferFrom(tokenReturnsTooMuchData, sender, recipient, amount);
    }
