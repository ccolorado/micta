...
export const deployTestContracts: () => Promise<TestDeployment> = deployments.createFixture(
  async ({ deployments, ethers, getNamedAccounts, getUnnamedAccounts, waffle }) => {
    ...
    const vaultAuthorizer = new Contract(VaultAuthorizer.address, AuthorizerArtifact.abi, deployerWallet);
    const vault = new Contract(Vault.address, VaultArtifact.abi, deployerWallet);
    const authenticator = await ethers.getContractAt(
      "GPv2AllowListAuthentication",
      GPv2AllowListAuthentication.address,
    );
    const settlement = await ethers.getContractAt("GPv2Settlement", GPv2Settlement.address);
    const vaultRelayer = await ethers.getContractAt("GPv2VaultRelayer", await settlement.vaultRelayer());

    return {
      deployer: deployerWallet,
      owner: findAccountWallet(allWallets, owner),
      manager: findAccountWallet(allWallets, manager),
      wallets: unnamedAccounts.map(account => findAccountWallet(allWallets, account)),
      weth,
      vaultAuthorizer,
      vault,
      authenticator,
